...
const generatedControllers = {
  ...
  init: router => {
    ...
    router.get(baseUrl + "/findByproject/:key", authorize([]), StudentController.findByproject);
  ...
  */
  findByproject: async (req, res) => {
    try {
      const result = await StudentModel.findByproject(req.params.key);
      res.json(result);
    } catch (err) {
      const safeErr = ErrorManager.getSafeError(err);
      res.status(safeErr.status).json(safeErr);
